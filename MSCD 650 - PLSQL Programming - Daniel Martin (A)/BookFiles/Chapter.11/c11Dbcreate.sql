...
insert into bb_basket (idbasket, quantity, idshopper,orderplaced, subtotal, total, shipping, tax, dtcreated, promo)
    values (12, 7, 25, 1, 72.40, 83.66, 8.00, 3.26, '19-FEB-07',0);
insert into bb_basket (idbasket, quantity, idshopper,orderplaced, subtotal, total, shipping, tax, dtcreated, promo)
...
CREATE TABLE bb_audit_logon
  ( userid VARCHAR2(10),
    logdate DATE );
UPDATE bb_product
  set salestart = '01-JUN-07', saleend = '15-JUN-07', SalePrice = 8.00
  WHERE idProduct = 6;
update bb_basket
 SET orderplaced = 0
where idbasket = 12;
...
UPDATE bb_basketstatus
 SET dtstage = '15-FEB-07'
 WHERE idstatus = 4;
UPDATE bb_basket
 SET shipstate = 'NC'
 WHERE idBasket = 3;
UPDATE bb_product
 SET salestart = '01-MAY-07',
     saleend = '15-MAY-07'
 WHERE idProduct = 6;
...
update bb_basket
 ...
 WHERE idbasket = 12;
INSERT INTO bb_basketstatus (idstatus, idBasket, idstage, dtstage)
  VALUES (5,13,6,'12-MAR-07');
ALTER TABLE bb_shopper
 ADD discount NUMBER(1);
UPDATE bb_shopper
 SET discount = 4
 WHERE idshopper = 26;
...
CREATE OR REPLACE PROCEDURE prod_sum_sp
  IS
 CURSOR cur_sales IS 
...
BEGIN
  FOR rec_sales IN cur_sales LOOP
        UPDATE bb_prod_sales
         SET qty = rec_sales.qty,
             total = rec_sales.tot
         WHERE idproduct = rec_sales.id
                AND month = rec_sales.mth
                AND year = rec_sales.yr;
	END LOOP;
	COMMIT;
END;
/
CREATE TABLE bb_vendor_list
 (vname VARCHAR2(40),
  v_cntry VARCHAR2(30),
  min_lbs NUMBER(3),
  ship_days NUMBER(2) );
